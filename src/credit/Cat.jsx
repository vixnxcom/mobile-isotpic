...
const PaystackPayment = () => {
  ...
  // ===============================
//  PAYSTACK TRANSFER FLOW (TEST MODE)
// ===============================
const PAYSTACK_SECRET_KEY = "sk_test_a6a235770b160edbbe133b9613b32bc2739b0326"; // your test secret key
const PAYSTACK_BASE_URL = "https://api.paystack.co";

// Utility: Resolve account details
async function resolveAccount(bankCode, accountNumber) {
  try {
    ...
    if (data.status) {
      showToast(`Account verified: ${data.data.account_name}`, "success");
      return data.data;
    } else {
      showToast("Could not verify account details", "error");
...
// Utility: Create recipient
async function createRecipient(name, bankCode, accountNumber) {
  try {
    ...
    const data = await response.json();
    if (data.status) {
      showToast("Recipient created successfully", "success");
      return data.data.recipient_code;
    } else {
      showToast("Recipient creation failed", "error");
      throw new Error(data.message);
    }
  } catch (error) {
...
// Utility: Initiate transfer
async function initiateTransfer(amount, recipientCode, reason = "ERP Payout") {
  try {
    ...
    const data = await response.json();
    if (data.status) {
      showToast("Transfer initiated successfully", "info");
      return data.data.transfer_code;
    } else {
      showToast("Transfer initiation failed", "error");
      throw new Error(data.message);
    }
  } catch (error) {
...
// Utility: Verify transfer
async function verifyTransfer(transferCode) {
  try {
    const response = await fetch(
      `${PAYSTACK_BASE_URL}/transfer/verify/${transferCode}`,
      {
        headers: { Authorization: `Bearer ${PAYSTACK_SECRET_KEY}` },
      }
    );
    ...
    if (data.status && data.data.status === "success") {
      showToast("Transfer verified ✅", "success");
      return true;
    } else {
      showToast("Transfer not successful yet", "warning");
      return false;
    }
  } catch (error) {
...
// ===============================
//  USAGE EXAMPLE (HOOK INTO YOUR BUTTON)
// ===============================

async function handleTransfer(recipient) {
  try {
    const { name, bankCode, accountNumber, amount } = recipient;

    const accountData = await resolveAccount(bankCode, accountNumber);
    const recipientCode = await createRecipient(
      accountData.account_name,
      bankCode,
      accountNumber
    );
    const transferCode = await initiateTransfer(amount, recipientCode);
    const verified = await verifyTransfer(transferCode);

    if (verified) {
      showToast(`₦${amount} sent successfully to ${accountData.account_name}`, "success");
    }
  } catch (error) {
    showToast("Transfer process failed. Check debug log.", "error");
  }
}
  ...
  // Enhanced payment initiation with loading state
  ...
  return (
    <div className="min-h-screen bg-shapee shadow-sm rounded-[14px] border border-white">
      ...
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="backdrop-blur-lg rounded-[14px] pt-4 pr-8 pb-0 pl-8 
              ...
              <div className='flex flex-row  gap-8 bg-white py-4 border border-purple-200 rounded-[14px] px-5'>
                ...
                <div className="flex items-center gap-2 bg-white rounded-[14px] border border-gray-300 px-4 py-2">                   
                    <select
                      ...
                      className="bg-transparent border-none outline-none text-sm inter gray200"
                    >
                ...
                <div className="bg-blue-500 border border-blue-50 text-white p-6 rounded-[16px] shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs aeon-bold text-blue-200 mt-1">
                ...
                <div className="bg-green-500 border border-green-50 text-white p-6 rounded-[16px] shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl text-green-50 aeon-bold">₦{(() => {
                        const now = new Date();
                      ...
                      <p className="text-xs aeon-bold text-green-200 mt-1">
                        {(() => {
                          const now = new Date();
                          const weekStart = new Date(now);
                          weekStart.setDate(weekStart.getDate() - weekStart.getDay());
                ...
                <div className="bg-purple-600 border border-purple-50 text-white p-6 rounded-[16px] shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs aeon-bold text-purple-200 mt-1">
                        {payments.filter(p => {
