...
const PaystackPayment = () => {
  ...
  // Primary function to execute the full flow: resolve -> create recipient -> initiate transfer -> verify
  
  const handleTransfer = async (recipient) => {
    // recipient from your sheet has fields: name, accountNumber, bank (value from dropdown)
    ...
    setIsLoading(true);
    logDebugInfo('handleTransfer_start', { recipient, amount });

    try {
      // Step 1: Resolve account (server-side via Apps Script)
      ...
      // resolved expected: { account_name, account_number, bank_code? }
      addToast(`Account verified: ${resolved.account_name}`, 'success');

      // Step 2: Create recipient on Paystack (server-side via Apps Script)
      addToast('Creating recipient on Paystack...', 'info');
      const recipientResult = await createRecipientViaAppsScript(resolved.account_name || recipient.name, recipient.bank, resolved.account_number || recipient.accountNumber);
      // recipientResult expected: { recipient_code }
      ...
      const transferData = await initiateTransferViaAppsScript(recipientCode, Math.round(amount * 100), `ERP Payout to ${resolved.account_name || recipient.name}`);
      // transferData expected: { transfer_code, status }
      ...
      // Step 4: Verify transfer status (you may want to poll here for pending transfers)
      const verification = await verifyTransferViaAppsScript(transferCode);

      // process verification
      if (verification.status === 'success' || verification.data?.status === 'success') {
        // Save payment to sheets just like test-mode payments
        const paymentObj = {
          id: Date.now(),
          recipientId: recipient.id,
          recipientName: resolved.account_name || recipient.name,
          amount: amount,
          status: 'success',
