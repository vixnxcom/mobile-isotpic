...
import { Plus, Edit, Trash2, Search, Package, TrendingUp, Calendar, PieChart, Download, Upload, Save, Trash, Printer, Cloud, Edit2, Edit3Icon, Edit2Icon } from 'lucide-react';
import { add, agenda, box, cloud, cloudd, coins, del, edit, filter, pack, paycard, report, status, text, today, trend } from '../assets';
...
export default function InventoryExpenseTracker() {
  ...
  const backupToGoogleSheets = async () => {
    ...
    try {
      ...
      // Update backup tracking for manual backup too
      localStorage.setItem('lastAutoBackupTime', Date.now().toString());
      localStorage.setItem('lastBackupDataHash', JSON.stringify(state.expenses));
  ...
  // NEW: Auto-backup to Google Sheets
  useEffect(() => {
    const autoBackupToSheets = async () => {
      // Only backup if there are expenses and we have internet
      if (state.expenses.length === 0 || !navigator.onLine) {
        return;
      }

      // Don't backup too frequently - wait at least 30 seconds between backups
      const lastBackupTime = localStorage.getItem('lastAutoBackupTime');
      const now = Date.now();
      if (lastBackupTime && (now - parseInt(lastBackupTime)) < 30000) {
        return; // Skip if last backup was less than 30 seconds ago
      }

      // Check if data has actually changed since last backup
      const lastBackupHash = localStorage.getItem('lastBackupDataHash');
      const currentHash = JSON.stringify(state.expenses);
      
      if (lastBackupHash === currentHash) {
        return; // Data hasn't changed, no need to backup
      }
      ...
      try {
        console.log('Auto-backup to Google Sheets...', { expensesCount: state.expenses.length });
        ...
        setConnectionStatus('error');
        // Don't show alert for auto-backup failures to avoid annoying users
      } finally {
        setIsSyncingToSheets(false);
      }
    };

    // Auto-backup when expenses change and we're online
    if (navigator.onLine && state.expenses.length > 0) {
      autoBackupToSheets();
    }
  }, [state.expenses]); // This will trigger whenever expenses change

  // NEW: Online/Offline Detection
  useEffect(() => {
    const handleOnline = () => {
      console.log('Internet connection restored - will auto-backup if needed');
      setConnectionStatus('connected');
      
      // Trigger auto-backup when coming back online
      if (state.expenses.length > 0) {
        setTimeout(() => {
          const autoBackupToSheets = async () => {
            // Only backup if there are expenses
            ...
            try {
              console.log('Online auto-backup to Google Sheets...', { expensesCount: state.expenses.length });
              ...
              console.log('✅ Online auto-backup successful');
              
            } catch (error) {
              console.error('Online auto-backup failed:', error);
              setConnectionStatus('error');
            } finally {
              setIsSyncingToSheets(false);
            }
  ...
  const getConnectionStatusText = () => {
    switch (connectionStatus) {
      case 'connected': 
        return isSyncingToSheets ? 'Syncing...' : 'Connected';
  ...
  // NEW: Function to calculate total amount when quantity or rate changes
  ...
  // NEW: Handle form field changes with automatic calculation
  const handleFormChange = (field, value) => {
    ...
    // Automatically calculate total amount when quantity or rate changes
    ...
    try {
      if (file.name.endsWith('.json')) {
        ...
        if (importedData.expenses && Array.isArray(importedData.expenses)) {
          ...
          alert('Invalid JSON file format. Please select a valid expense backup file.');
        }
      } else if (file.name.endsWith('.csv')) {
        // Handle CSV import
        const csvData = e.target.result;
        const expenses = parseCSVData(csvData);
        if (expenses.length > 0) {
          if (window.confirm(`Import ${expenses.length} expenses from CSV? This will replace your current data.`)) {
            syncDispatch({ type: 'LOAD_DATA', payload: { expenses } });
            alert('Data imported successfully from CSV!');
          }
        } else {
          alert('No valid expense data found in CSV file.');
        }
      } else {
...
const parseCSVData = (csvText) => {
  ...
  // Expected CSV format (matching your export format):
  // Date,Product Name,SKU,Category,Quantity,Total Amount (NGN)
  const expenses = [];
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (values.length >= 6) {
      const expense = {
        ...
        date: values[0] || localDateStr(),
        productName: values[1] || '',
        sku: values[2] || '',
        category: values[3] || '',
        quantity: parseInt(values[4]) || 1,
        totalAmount: parseFloat(values[5].replace(/[^0-9.-]+/g, "")) || 0 // Remove currency symbols
      };
      
      // Only add if we have basic required data
      if (expense.productName && expense.totalAmount > 0) {
...
const parseCSVLine = (line) => {
  ...
  const filterByPeriod = (expenses, period) => {
    switch (period) {
      ...
      case 'alltime': // Add this case
        return expenses; // Return all expenses without date filtering
      default:
        return expenses;
    }
  };
  ...
  state.expenses.forEach(exp => {
    if (!categoryTotals[exp.category]) {
      categoryTotals[exp.category] = { 
        ...
        weekly: 0, 
        monthly: 0,
        alltime: 0 // Add alltime tracking
      };
    }
    ...
    // Add alltime total (always include every expense)
  ...
  return {
    ...
    alltime: state.expenses.reduce((sum, exp) => sum + exp.totalAmount, 0), // Add alltime total
  ...
  const resetForm = () => {
    setExpenseForm({
      ...
      category: '',
      quantity: '',
      rate: '', // ADDED: Include rate field in reset
      totalAmount: '',
      date: localDateStr()
    });
  ...
  const openModal = (expense = null) => {
    ...
    if (expense) {
      setExpenseForm({
        ...
        rate: '', // You might want to calculate this or store it separately
  ...
  // Add this function to monitor storage
const getStorageUsage = () => {
  try {
    const data = localStorage.getItem('expenseTrackerData');
    if (!data) return { usedKB: 0, usedMB: 0, percentage: 0 };
    
    const usedBytes = new Blob([data]).size;
    const usedKB = (usedBytes / 1024).toFixed(2);
    const usedMB = (usedBytes / 1024 / 1024).toFixed(2);
    const percentage = ((usedBytes / (5 * 1024 * 1024)) * 100).toFixed(1); // 5MB limit
    ...
    return { usedKB: 0, usedMB: 0, percentage: 0 };
  }
};
  ...
  <label 
    htmlFor="import-file"
    className="border border-pink-500 text-pink-500 px-3 py-2 rounded-[14px] bg-white
              hover:!bg-pink-500 hover:!text-white flex items-center gap-2 text-xs
              cursor-pointer transition-colors duration-200"
    title="Import JSON or CSV backup file"
  >
...
<div className="bg-white p-4 rounded-[14px] border border-green-200">
  <p className="text-green-800 text-sm inter">
    ...
    {/* Add auto-backup status */}
    <br />
    <span className={`text-xs ${navigator.onLine ? 'text-green-600' : 'text-orange-300'}`}>
      {navigator.onLine ? 
        '✓ Internet connected - auto-backup enabled' : 
        '⚠ Offline - will auto-backup when connection returns'}
    </span>
    
    {/* Add Storage Usage Monitoring */}
    <div className='mt-5 intermid gray200'>
       Storage Usage:  
    <span className=" inter text-green-600  ">
    {getStorageUsage().usedMB}MB used ({getStorageUsage().percentage}% of ~5MB limit)
      {getStorageUsage().percentage > 80 && (
        <span className="text-orange-600"> • Consider annual backup soon</span>
      )}
    </span>
    </div>
  </p>
...
{Object.keys(calculateExpenses.categoryTotals).length > 0 ? (
  <div className="bg-white p-6 rounded-[14px] border border-gray-200">
    ...
    {/* Add this new function to process the top 6 categories */}
    {(() => {
      // Process categories for top 6 display
  ...
  const renderExpenses = () => (
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-[14px] border border-green-200 hover:shadow-lg transition-all duration-300 hover:scale-[1.02]">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 inter gray200">
          <input
            type="text"
        ...
        {/* UPDATED: Show both rate and total amount preview */}
        {(expenseForm.rate || expenseForm.totalAmount) && (
          ...
          <button
            type="submit"
            className="bg-green-600 mt-4 text-white px-4 py-3 rounded-[14px] hover:bg-gradient-to-r from-green-600 to-blue-600 text-md w-full intermid md:col-span-2 lg:col-span-6 transform hover:scale-102 transition-all"
          >
            Add Expense
          </button>
      </div>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-6 rounded-lg w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 inter gray200">
            ...
            {/* UPDATED: Show both rate and total amount preview in modal */}
            {(expenseForm.rate || expenseForm.totalAmount) && (
...
>
  ...
  <option value="alltime">All Time</option> {/* Add this line */}
</select>
          </div>
        </div>
